...
func newMinioManager(s3Config config.S3Config) *minioManager {
	mgr := new(minioManager)
	mgr.s3Config = s3Config
	return mgr
}

func (mgr *minioManager) Connect() error {
	client, err := minio.New(mgr.s3Config.URL, &minio.Options{
		Creds:  credentials.NewStaticV4(mgr.s3Config.AccessKey, mgr.s3Config.SecretKey, ""),
		Secure: mgr.s3Config.Secure,
	})
	if err != nil {
		return err
	}
