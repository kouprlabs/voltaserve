...
from ..errors import NotFoundError, UnknownApiError
from ..log import base_logger
from ..models import VersionRequest, VersionResponse

overview_api_router = APIRouter(prefix="/overview", tags=["overview"], dependencies=[Depends(JWTBearer())])
        ...
        async with sess.get(
            ...
            if resp.status == 200:
                ...
                response["latestVersion"] = max(
                    tag["name"] for tag in tags["results"] if tag["digest"] == latest_digest and tag["name"] != "latest"
                )
                response["location"] = (
...
@overview_api_router.get(path="/version/internal", responses={status.HTTP_200_OK: {"model": VersionResponse}})
    ...
    try:
        ...
        async with ClientSession() as sess:
            ...
            response["updateAvailable"] = (
                ...
                if response["currentVersion"] != "" and response["latestVersion"] != "UNKNOWN"
                else None
            )
