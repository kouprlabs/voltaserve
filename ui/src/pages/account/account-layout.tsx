...
import { Avatar, Button, Heading, IconButton, Tab, TabList, Tabs } from '@chakra-ui/react'
...
const AccountLayout = () => {
  ...
  const { data: user, isLoading: userIsLoading, error: userError, mutate } = UserAPI.useGet(swrConfig())
      ...
      {userIsReady ? (
        <>
          ...
          <div className={cx('flex', 'flex-row', 'gap-2.5')}>
            <div className={cx('flex', 'flex-col', 'gap-2', 'items-center', 'w-[250px]')}>
              <div className={cx('flex', 'flex-col', 'gap-2', 'items-center')}>
                ...
                <Heading className={cx('text-center', 'text-heading')}>{truncateEnd(user.fullName, 50)}</Heading>
              </div>
            ...
            <div className={cx('w-full', 'pb-1.5')}>
              <Tabs variant="solid-rounded" colorScheme="gray" index={tabIndex} className={cx('pb-2.5')}>
                <TabList>
                  ...
                  <Tab onClick={() => navigate('/account/invitation')}>
                    <div className={cx('flex', 'flex-row', 'items-center', 'gap-0.5')}>
                      ...
                      {invitationCount && invitationCount > 0 ? (
                        <NumberTag isActive={tabIndex === 1}>{invitationCount}</NumberTag>
                      ) : null}
                    </div>
                  </Tab>
                </TabList>
          ...
          <AccountEditPicture open={isImageModalOpen} user={user} onClose={() => setIsImageModalOpen(false)} />
        </>
      ) : null}
    </>
  )
