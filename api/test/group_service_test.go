...
func (s *GroupServiceSuite) TestFindGroup() {
	...
	foundGroup, err := s.service.Find(group.ID, s.users[0].GetID())
	s.Require().NoError(err)
	s.NotNil(foundGroup)
	s.Equal(group.ID, foundGroup.ID)

	// Test finding a non-existent group
	foundGroup, err = s.service.Find("non-existent-group-id", s.users[0].GetID())
	s.Require().Error(err)
...
func (s *GroupServiceSuite) TestListGroups() {
	...
	list, err := s.service.List(service.GroupListOptions{
		OrganizationID: s.org.ID,
		Page:           1,
		Size:           10,
	}, s.users[0].GetID())
...
func (s *GroupServiceSuite) TestProbeGroups() {
	...
	// Test probing groups
	probe, err := s.service.Probe(service.GroupListOptions{
		OrganizationID: s.org.ID,
		Page:           1,
		Size:           10,
	}, s.users[0].GetID())
...
func (s *GroupServiceSuite) TestPatchGroupName() {
	...
	group, err = s.service.PatchName(group.ID, "group (edit)", s.users[0].GetID())
	s.Require().NoError(err)
	s.NotNil(group)
	s.Equal("group (edit)", group.Name)

	// Test patching a non-existent group
	group, err = s.service.PatchName("non-existent-group-id", "group", s.users[0].GetID())
	s.Require().Error(err)
	s.Equal(errorpkg.NewGroupNotFoundError(nil).Error(), err.Error())
	s.Nil(group)
}
...
func (s *GroupServiceSuite) TestDeleteGroup() {
	...
	err = s.service.Delete(group.ID, s.users[0].GetID())
	s.Require().NoError(err)

	// Test finding the deleted group
	group, err = s.service.Find(group.ID, s.users[0].GetID())
	s.Require().Error(err)
	s.Equal(errorpkg.NewGroupNotFoundError(nil).Error(), err.Error())
	s.Nil(group)

	// Test deleting a non-existent group
	err = s.service.Delete("non-existent-group-id", s.users[0].GetID())
	s.Require().Error(err)
	s.Equal(errorpkg.NewGroupNotFoundError(nil).Error(), err.Error())
}
...
func (s *GroupServiceSuite) TestAddMember() {
	...
	err = s.service.AddMember(group.ID, s.users[1].GetID(), s.users[0].GetID())
	s.Require().NoError(err)

	// Test adding a non-existent member
	err = s.service.AddMember(group.ID, s.users[2].GetID(), s.users[0].GetID())
	s.Require().Error(err)
...
func (s *GroupServiceSuite) TestRemoveMember() {
	...
	err = s.service.RemoveMember(group.ID, s.users[1].GetID(), s.users[0].GetID())
	s.Require().NoError(err)
	...
	err = s.service.RemoveMember(group.ID, s.users[0].GetID(), s.users[0].GetID())
	s.Require().Error(err)
	s.Equal(
