...
func (r *FileRouter) Patch(c *fiber.Ctx) error {
	...
	hasEnoughSpace, err := r.workspaceSvc.HasEnoughSpaceForByteSize(file.Workspace.ID, fh.Size, userID)
	if err != nil {
		return err
	}
	if !hasEnoughSpace {
...
func (r *FileRouter) PatchFromS3(c *fiber.Ctx) error {
	...
	hasEnoughSpace, err := r.workspaceSvc.HasEnoughSpaceForByteSize(file.Workspace.ID, size, userID)
	if err != nil {
		return err
	}
	if !hasEnoughSpace {
